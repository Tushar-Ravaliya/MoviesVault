<?php
$page_title = "Edit aboutus Content";
ob_start();
...
if (!$aboutus_content) {
    $insert_query = "INSERT INTO aboutus_content (id, title, content, image_path) 
                     VALUES (1, 'Welcome to Our Site', 'This is the default aboutus content.', 'default_image.jpg')";
...
// Process form submission for updating aboutus content
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $title = trim($_POST['title']);
    $content = trim($_POST['content']);
    $current_image = $aboutus_content['image_path'];
    ...
    if (isset($_FILES['image']) && $_FILES['image']['size'] > 0) {
        ...
        $new_filename = "aboutus_image_" . time() . "." . $file_extension;
    ...
    if (empty($title)) {
        ...
        if (mysqli_query($conn, $update_query)) {
            $success_message = "aboutus content updated successfully";
    ...
    <div class="mt-8 w-10/12">
            ...
            <div class="p-6 bg-white rounded-md border border-gray-300">
                ...
                <form method="post" action="" enctype="multipart/form-data">
                    <div class="gap-6 mt-4">
                        <div class="mb-4">
                            <label class="text-gray-700 font-medium block mb-2" for="title">aboutus Title</label>
                            <input
                                ...
                                type="text"
                                name="title"
                                value="<?php echo htmlspecialchars($aboutus_content['title']); ?>"
                                required>
                        </div>

                        <div class="mb-4">
                            <label class="text-gray-700 font-medium block mb-2" for="content">aboutus Content</label>
                            <textarea
                                class="w-full px-4 py-2 border border-gray-400 outline-none rounded-md focus:border-blue-600 focus:ring focus:ring-opacity-40 focus:ring-blue-500"
                                name="content"
                                rows="6"
                                required><?php echo htmlspecialchars($aboutus_content['content']); ?></textarea>
                        </div>

                        <div class="mb-4">
                            <label class="text-gray-700 font-medium block mb-2" for="image">aboutus Image</label>
                            <?php if (!empty($aboutus_content['image_path'])): ?>
                                <div class="mb-3">
                                    <p class="text-sm text-gray-600 mb-2">Current Image:</p>
                                    <img src="../../public/Images/<?php echo htmlspecialchars($aboutus_content['image_path']); ?>"
                                        alt="Current aboutus Image"
                        ...
                        <div class="flex justify-end">
                            <button type="submit"
                                class="px-5 py-2 text-center bg-blue-600 text-white rounded transition duration-300 hover:bg-blue-700 focus:outline-none">Update aboutus</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="p-6 mt-10 bg-white rounded-md border border-gray-300">
            ...
            <div class="border border-gray-200 p-4 rounded-md">
                <h2 class="text-2xl font-bold mb-3"><?php echo htmlspecialchars($aboutus_content['title']); ?></h2>
                <div class="flex flex-col md:flex-row gap-4">
                    ...
                    <div class="md:w-2/3">
                        <p class="text-gray-700"><?php echo nl2br(htmlspecialchars($aboutus_content['content'])); ?></p>
                    </div>
                </div>
            </div>
        </div>
