...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $theater_id = $conn->real_escape_string($_POST['theater']);
...
$theaters = [];
$theaters_result = $conn->query("SELECT id, title FROM theaters WHERE status = 'Active'");
...
<script>
    ...
    // Function to load screens based on theater selection
    function loadScreens() {
        ...
        fetch(`get_screens.php?theater_id=${theaterId}`)
            ...
            .then(data => {
                ...
                data.forEach(screen => {
                    ...
                    option.textContent = `${screen.screen_name} (${screen.screen_type}, ${screen.seating_capacity} seats)`;
